...
<head>
  ...
  <title>Data Login - SiPeduli</title>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .no-cell { color:var(--muted); font-size:0.8rem; font-weight:600; }
...
<aside class="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <a href="biodata_user.php" class="nav-item active">
...
<div class="main">
  <header class="topbar">
    <div class="topbar-title">Data Login</div>
  ...
  <div class="content">
    <div class="stats-row">
      ...
      <div class="stat-card green">
        ...
        <div class="stat-value">
          <?php
            $q = $koneksi->query("SELECT COUNT(*) as c FROM users WHERE telepon IS NOT NULL AND telepon != ''");
            echo $q ? $q->fetch_assoc()['c'] : '—';
          ?>
        </div>
        <div class="stat-desc">Nomor terisi</div>
      </div>
      <div class="stat-card purple">
        <div class="stat-label">Dengan Alamat</div>
        <div class="stat-value">
          <?php
            $q2 = $koneksi->query("SELECT COUNT(*) as c FROM users WHERE alamat IS NOT NULL AND alamat != ''");
            echo $q2 ? $q2->fetch_assoc()['c'] : '—';
          ?>
        </div>
        <div class="stat-desc">Alamat terisi</div>
      </div>
    </div>
    ...
    <div class="table-section">
      <div class="table-header">
        ...
        <span class="table-count"><?= $total ?> user</span>
      </div>
      ...
      <table>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td><span class="badge badge-blue">@<?= htmlspecialchars($row['username']) ?></span></td>
            <td class="muted-cell"><?= htmlspecialchars($row['email']) ?></td>
            <td class="muted-cell"><?= isset($row['alamat']) && $row['alamat'] ? htmlspecialchars($row['alamat']) : '<span style="color:#334155">—</span>' ?></td>
            <td class="muted-cell"><?= isset($row['telepon']) && $row['telepon'] ? htmlspecialchars($row['telepon']) : '<span style="color:#334155">—</span>' ?></td>
          </tr>
          <?php endwhile; else: ?>
          <tr><td colspan="6" style="text-align:center;color:var(--muted);padding:40px;">Belum ada data user.</td></tr>
          <?php endif; ?>
        </tbody>
      </table>
    </div>
